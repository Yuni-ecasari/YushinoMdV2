...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode }) => {
 ...
 let ftroli = { key: { remoteJid: 'status@broadcast', participant: '0@s.whatsapp.net' }, message: { orderMessage: { itemCount: 9999999999, status: 1, surface: 1, message: `${command} ${type}`, orderTitle: wm, sellerJid: '0@s.whatsapp.net' } } }
  let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Fajar Alfarizi⸙;;;\nFN:Fajar Alfarizi⸙\nORG:Fajar Alfarizi⸙\nTITLE:\nitem1.TEL;waid=6281333782061:+62 813-3378-2061\nitem1.X-ABLabel:Fajar Alfarizi⸙\nX-WA-BIZ-DESCRIPTION:${htjava} JANGAN DISPAM!!\nX-WA-BIZ-NAME:Fajar Alfarizi⸙\nEND:VCARD`
//------- NOMOR
...
let teksbio = `┆⫹⫺ 💌 Nama : Felix
┆⫹⫺ ✉️ Nama RL : Fajar Alfarizi
┆⫹⫺ ♂️ Gender : Pria
┆⫹⫺ 🕋 Agama : Islam
┆⫹⫺ ⏰ Tanggal lahir : 17 - 03 - 2007
┆⫹⫺ 🎨 Umur : 15
┆⫹⫺ 🧮 Kelas : X
┆⫹⫺ 🧩 Hobby : 𝙼𝚘𝚍𝚒𝚏𝚒𝚔𝚊𝚜𝚒 𝚙𝚎𝚛𝚊𝚗𝚐𝚔𝚊𝚝 𝚕𝚞𝚗𝚊𝚔
┆⫹⫺ 💬 Sifat : ASIK,BAIK, TIDAK RAMAH
┆⫹⫺ 🗺️ Tinggal : Indonesia, jawa Timur Bojonegoro
└––––––––––––·•
`
let ppown2 = await conn.profilePictureUrl(nomorown2 + '@s.whatsapp.net', 'image').catch(_ => hwaifu[1]) 
let teksbio2 = `┆⫹⫺ 💌 Nama : Eca
┆⫹⫺ ✉️ Nama RL : Eca Sari
┆⫹⫺ ♂️ Gender : Wanita
┆⫹⫺ 🕋 Agama : Islam
┆⫹⫺ ⏰ Tanggal lahir : 07 - 07 - 2007
┆⫹⫺ 🎨 Umur : 15
┆⫹⫺ 🧮 Kelas : X
┆⫹⫺ 🧩 Hobby : Banyak
...
`
  ...
  let tek = `⟣⟞⟚⟝ 〨⎣ *Nᴏᴛᴇ* ⎤〨 ⟞⟚⟝⟢
┆
𐚀 Owner Tidak Menerima Chat Aneh
𐚀 Owner Berhak Block/Ban Siapa Saja
𐚀 Gak Respon Kalo Minta Sc
𐚀 Etika Atittudenya Dipake
𐚀 Chat Cewe Gua Aneh Aneh Gua Buat Tidur Lu
...
{title: "⸙ Biodata Fajar", rowId: ".owner bio"},
{title: "⸙ Biodata Eca", rowId: ".owner bio2"},
...
await conn.sendMessage(m.chat, { contacts: { displayName: wm, contacts: [{ vcard }] }}, { quoted: ftroli })
 ...
 conn.send2ButtonImg(m.chat, fs.readFileSync('./media/own.jpg').buffer, teksnomor, `Halo ${tag}\nIᴛᴜ Oᴡɴᴇʀ Kᴜ⫺⫰⫹\n` + botdate, `Sewa Bot`, `.sewa`, 'Menu', `.menu`, ftroli, { contextInfo: { externalAdReply: { showAdAttribution: true,
   ...
   conn.sendButton(m.chat, '┍┈༺ *Biodata Fajar* ༻', teksbio, ppown, [
                ['Sewa Bot', `${usedPrefix}sewa`],
                ['Menu', `${usedPrefix}menu`]
            ], m)
            break
          ...
          conn.sendButton(m.chat, '┍┈༺ *biodata Eca* ༻', teksbio2, ppown2, [
                ['Sewa Bot', `${usedPrefix}sewa`],
                ['Menu', `${usedPrefix}menu`]
            ], m)
            break
